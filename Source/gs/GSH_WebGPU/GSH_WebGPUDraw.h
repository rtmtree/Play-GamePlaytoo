...
namespace GSH_WebGPU
{
	...
	public:
		...
		{
			uint32 primitiveType : 2;     // 0=triangle, 1=line, 2=point

			uint32 hasTexture              : 1;
			uint32 textureHasAlpha         : 1;
			uint32 textureBlackIsTransparent : 1;
			uint32 textureFunction         : 2;  // 0=MODULATE, 1=DECAL, 2=HIGHLIGHT, 3=HIGHLIGHT2
			...
			// texture / framebuffer pixel formats (for render pass compatibility)
			uint32 textureFormat    : 6;
			uint32 colorFormat      : 6;
			uint32 depthFormat      : 6;
		};
		static_assert(sizeof(PIPELINE_CAPS) <= sizeof(PipelineCapsInt), "PIPELINE_CAPS too big");
	...
	private:
		...
		// Current state — updated by Set* calls
		uint32 m_fbBufAddr   = 0;
		uint32 m_fbBufWidth  = 0;
		uint32 m_depthBufAddr  = 0;
		uint32 m_depthBufWidth = 0;
		uint32 m_texBufAddr  = 0;
